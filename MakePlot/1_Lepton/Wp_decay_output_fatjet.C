...
void Wp_decay_test2_bb(const char *inputFile)
{
  ...
  // Output root Files
  TFile* outfile = TFile::Open(Form("output_bb_%s", inputFile), "recreate");
  ...
  TH1 *fatjetM = new TH1F("fatjetMaiss", "fatjetMass", 100, 0., 500.);
  TH1 *fatjetM1_s= new TH1F("fatjetMass1_signal", "fatjetMass1_signal", 100, 0., 500.);
  TH1 *fatjetM2_s= new TH1F("fatjetMass2_signal", "fatjetMass2_signal", 100, 0., 500.);
  TH1 *fatjetM_s= new TH1F("fatjetMass_signal", "fatjetMass_signal", 100, 0., 500.);
  ...
  for(Int_t entry = 0; entry < numberOfEntries; ++entry)
  {
	...
	if (branchJet->GetEntries() > 0 && branchElectron->GetEntries() > 0){
		...
		if (Jbc_index.size()<1) continue;             
			...
			FJ1 = jet1->P4();                         
			fatjetM1->Fill(FJ1.M());
			if (FJ1.M()<105. || FJ1.M()>135.) continue;
			else{
				fatjetM1_s->Fill(FJ1.M());
			}
		}
		Jbc_index.clear();	
	}
  ...
  fatjetM_s->Clone("fatjetM1_s");
  fatjetM_s->Add(fatjetM2_s);    
...
void Wp_decay_output_fatjet(){

	Wp_decay_test2_bb("HVT_QQToWpToNuL2b_4TeV.root");
	Wp_decay_test2_bb("HVT_QQToWpToNuL2Q_4TeV.root");
	Wp_decay_test2_bb("BKG_ttWJetToLNu_LO_MLM_cut3.root");
    Wp_decay_test2_bb("BKG_WZToLNu2Q_LO_MLM_cut3.root");
	Wp_decay_test2_bb("BKG_WZToL3Nu_LO_MLM_cut3.root");
	Wp_decay_test2_bb("BKG_WWToLNu2Q_4f_LO_MLM_cut3.root");
	Wp_decay_test2_bb("BKG_WJetsToLNu_LO_MLM_cut3.root");
	Wp_decay_test2_bb("BKG_ST_t-channel_4f_inclusiveDecays_LO_cut3.root");
	Wp_decay_test2_bb("BKG_ST_s-channel_4f_leptonDecays_LO_cut3.root");
	Wp_decay_test2_bb("BKG_WH_HToBB_WToLNu.root");

...
void Draw_Photon_DeltaPhi(){
	...
	for(int fileno = 0; fileno < 4; fileno++){
		...
		double DeltaPhi_Photons;
		Photontree->SetBranchAddress("DeltaPhi_Photons", &DeltaPhi_Photons);

		TH1D *P = new TH1D(Form("P%d", fileno)," ", 50, -3.2, 3.2);
		P->SetStats(0);     
		P->SetLineWidth(3);
		P->GetXaxis()->SetTitle("#Delta#phi(#gamma, #gamma)");

...
void Wp_decay_graph_output(const char *inputFile)
{
  ...
  // Output root Files
  TFile* outfile = TFile::Open(Form("output_Loose_%s", inputFile), "recreate");
  ...
  //Photontree->Branch("Total_Entries", &Total_Entries, "Total_Entries/I");
  ...
  for(Int_t entry = 0; entry < numberOfEntries; ++entry) {
	...
	if(branchPhoton->GetEntries()>1){
		if(branchMuon->GetEntries() > 0 && branchElectron->GetEntries() == 0){
			...
			// For direction restricton, we ask R(photon,e) > 0.2, R(photon, mu) > 0.2, for each of the leptons(Updated on 09/04/2020).
...
// Main output process
void Wp_decay_output_DiPhotonMass_Loose(){
	
	
	for(int file_no = 0; file_no < 8; file_no++){
		int cut_Max = (file_no+1)*100;
		string file_name = Form("BKG_WGToLNu2G_01Jet_LO_MLM_PTcut_M%d.root", cut_Max);
		const char *c = file_name.c_str();
		Wp_decay_graph_output(c);
		cout << "output done" << endl;
	}
	
	
	Wp_decay_graph_output("BKG_WGToLNu2G_01Jet_LO_MLM_PTcut_MInf_50k.root");     
	cout << "output done" << endl; 

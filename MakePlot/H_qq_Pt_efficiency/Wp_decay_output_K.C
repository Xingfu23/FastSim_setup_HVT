...
void Wp_decay_graph_output(const char *inputFile)
{
  ...
  TH1 *DiPhotonPt   = new TH1F("DiPhotonPt", "DiPhotonPt", 100, 0., 1000.);
  ...
  for(Int_t entry = 0; entry < numberOfEntries; ++entry)
  {
	...
	if(branchPhoton->GetEntries()>1){
		if(branchMuon->GetEntries() > 0 && branchElectron->GetEntries() == 0){
			...
			// Take 1st and 2nd Photon, reconstructing diphoton mass and momentum
			...
			DiPhotonPt->Fill(((photon1->P4())+(photon2->P4())).Pt());
			...
			// Collecting Missing Et information
...
void Wp_decay_output_K(){
	...
	Wp_decay_graph_output("HVT_QQToWpToNuL2gamma_Wprime_4TeV.root");
	cout << "output done" << endl;
	Wp_decay_graph_output("BKG_WH_HToGammagamma_WToLNu.root");
	cout << "output done" << endl;
